...
class SAC:
    def __init__(self, nS,nA, lr, gamma,batch_size):
        self.state_shape = nS
        self.n_actions = nA
        self.lr = lr
        self.gamma = gamma
    ...
    def train(self,replay_buffer):
            ...
            goals,states, rewards,  actions, next_states ,dones= replay_buffer.get_batch()
            goals,states, rewards,  actions, next_states ,dones=th.tensor(goals),th.tensor(states), th.tensor(rewards),  th.tensor(actions), th.tensor(next_states) ,th.tensor(dones)
            
            goal_states = th.cat([states, goals], dim=1)
            goal_next_states = th.cat([next_states, goals], dim=1)
    ...
    def get_action(self, states, do_greedy=False):
